...
class UDP(Expt):
    def start(self):
        ...
        self.hlist.rmrf(e(""))
        self.hlist.mkdir(e("logs"))

        if sniffer:
            self.sniffer.rmrf(e("", tmpdir=config['SNIFFER_TMPDIR']))
            self.sniffer.mkdir(e("logs", tmpdir=config['SNIFFER_TMPDIR']))
        ...
        if self.opts("rl") == "htb":
            ...
            if self.opts("num_class") is not None:
                ...
                self.client.htb_class_filter_output(e('logs'))
        ...
        self.client.start_cpu_monitor(e('logs'))
        self.client.start_bw_monitor(e('logs'))
        if self.opts("rl") == "qfq":
            self.client.start_qfq_monitor(e('logs'))
        self.client.start_mpstat(e('logs'))
        self.client.set_mtu(self.opts("mtu"))
        if sniffer:
            self.sniffer.start_sniffer_delayed(e('logs', tmpdir=config['SNIFFER_TMPDIR']),
    ...
    def stop(self):
        ...
        if self.opts("sniffer"):
            self.sniffer.stop_sniffer(e('logs', tmpdir=config['SNIFFER_TMPDIR']))
            # Hack to just tar the sniffer log file alone without the directory.
            # We specify the logfilename as the src_dir
            #self.sniffer.copy_local(src_dir = e('pkt_snf.txt', tmpdir=config['SNIFFER_TMPDIR']),
            #                        exptid = self.opts("exptid") + "-snf",
            #                        tmpdir = e('', tmpdir=config['SNIFFER_TMPDIR']))
